...
public:
   ...
   */
  Node(T x = 0, T y = 0, double g = 0.0, double h = 0.0, int id = 0,
       int pid = 0)
      : x_(x), y_(y), g_(g), h_(h), id_(id), pid_(pid){};
  Node(const Node &n)
      : x_(n.x()), y_(n.y()), g_(n.g()), h_(n.h()), id_(n.id()),
        pid_(n.pid()){};
   ...
   */
  Node operator+(const Node &n) const { return Node(x_ + n.x(), y_ + n.y()); };
   ...
   */
  bool operator==(const Node &n) const { return x_ == n.x() && y_ == n.y(); };
  ...
  struct compare_cost {
    ...
    bool operator()(const Node &n1, const Node &n2) const {
      return (n1.g() + n1.h() > n2.g() + n2.h()) ||
             ((n1.g() + n1.h() == n2.g() + n2.h()) && (n1.h() > n2.h()));
    };
  };

  /**
   * @brief Struct created to encapsulate function compare coordinates between 2
   * Nodes. Used in with multiple algorithms and classes
   */
...
protected:
  ...
  double g_, h_; // g value, cost to reach this node. h value, heuristic cost to
                 // reach the goal
