...
public:
   ...
   */
  bool inCollision(const unsigned int &x, const unsigned int &y,
  ...
  /**
   * @brief 1d-collision checker Bresenham: to check if there is any obstacle
   * between pt1 and pt2
   * @param pt1 point1
   * @param pt2 point2
   * @param func_is_obs the function to judge whether the position (x, y) is
   * obstacle
   * @return true if collision occurs, else false
   */
  template <typename Point, typename F_is_obs>
  static bool BresenhamCollisionDetection(const Point &pt1, const Point &pt2,
                                          F_is_obs func_is_obs) {
    int s_x = (pt1.x() - pt2.x() == 0)
                  ? 0
                  : (pt1.x() - pt2.x()) / std::abs(pt1.x() - pt2.x());
    int s_y = (pt1.y() - pt2.y() == 0)
                  ? 0
                  : (pt1.y() - pt2.y()) / std::abs(pt1.y() - pt2.y());
