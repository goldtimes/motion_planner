 ...
 */
bool CollisionChecker::inCollision(const unsigned int &x, const unsigned int &y,
 ...
 * @return true if collision occurs, else false
 */
bool CollisionChecker::inCollision(const unsigned int &i,
                                   const bool &traverse_unknown) {
  ...
  return center_cost >=
         costmap_2d::INSCRIBED_INFLATED_OBSTACLE * obstacle_factor_;
}
...
bool CollisionChecker::isInsideMap(const unsigned int &i) {
  unsigned int size =
      static_cast<unsigned int>(costmap_ros_->getCostmap()->getSizeInCellsX() *
